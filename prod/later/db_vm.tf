...
# availabilty set
resource "azurerm_availability_set" "eu2_prd_db_vm_avset" {
  name                = var.vm_avset
  location            = var.location
...
# NIC
resource "azurerm_network_interface" "eu2_prd_db_vm_nic" {
...
#VM
resource "azurerm_virtual_machine" "eu2_prd_db_vm" {
  name                  = var.vm_name
  location              = var.location
  resource_group_name   = azurerm_resource_group.db_rsg.name
  network_interface_ids = [azurerm_network_interface.eu2_prd_db_vm_nic.id]
  availability_set_id   = azurerm_availability_set.eu2_prd_db_vm_avset.id
  vm_size               = var.vm_size
...
output "eu2_prd_db_vm_name" {
  value = azurerm_virtual_machine.eu2_prd_db_vm.name
}

output "eu2_prd_db_vm_username" {
  value = "${lower(var.vm_name)}-adm"
}

output "eu2_prd_db_vm_password" {
  value = var.admin_password
}

output "eu2_prd_db_vm_private_ips" {
  value = azurerm_network_interface.eu2_prd_db_vm_nic.private_ip_address
}

output "eu2_prd_db_vm_rsg" {
  value = azurerm_virtual_machine.eu2_prd_db_vm.resource_group_name
}

output "eu2_prd_db_vm_location" {
  value = azurerm_virtual_machine.eu2_prd_db_vm.location
}
